import 'package:flutter/material.dart';

const Color maunen = Color(0xFFE6E4E6);
const NetworkImage avatar = NetworkImage(
    'https://th.bing.com/th/id/OIP.r7YawY1hvB1sWIuhGZLtuQHaJQ?pid=ImgDet&rs=1');

const TextStyle tTitle =
    TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: Colors.white);
const TextStyle tHeadline =
    TextStyle(fontSize: 18, fontWeight: FontWeight.w300, color: Colors.black87);
const TextStyle tnormal =
    TextStyle(fontWeight: FontWeight.normal, fontSize: 14, color: Colors.black);

const TextStyle tChatName = TextStyle();
